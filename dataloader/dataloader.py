...
def get_testloader(args, session):
    ...
    if args.dataset == 'FMC':
        ...
        testset = args.Dataset.FSD(root=args.dataroot, phase="test",
                                      index=class_new, k=None, args=args)
...
def get_pretrain_dataloader(args):
    ...
    if args.dataset == 'FMC':
        ...
        trainset = args.Dataset.FSD(root=args.dataroot, phase="train",
                                             index=class_index, base_sess=True, args=args)
        valset = args.Dataset.FSD(root=args.dataroot, phase="val", index=class_index, base_sess=True, args=args)
...
def get_base_dataloader_stdu(args):
    ...
    # DataLoader(test_set, batch_sampler=sampler, num_workers=8, pin_memory=True)
        ...
        valset = args.Dataset.FSD(root=args.dataroot, phase='val', index=class_index, k=None, args=args) # k is same as new_loader's testset k
...
def get_base_dataloader_as_dfsl(args):
    class_index = np.arange(args.num_base)
    ...
    if args.dataset == 'FMC':
        ...
        trainset = args.Dataset.FSD(root=args.dataroot, phase='train', index=class_index, k=None, args=args)
        valset = args.Dataset.FSD(root=args.dataroot, phase='val', index=class_index, k=None, args=args) # k is same as new_loader's testset k
    train_sampler = DFSLTrainCategoriesSampler(label=trainset.targets, n_batch=args.episode.train_episode, 
                                            n_cls=args.num_base, n_inc_cls=args.episode.episode_way,
                                            n_shot=args.episode.episode_shot, n_query=args.episode.episode_query)
...
def get_new_dataloader(args, session):
    ...
    if 'fsd' in args.dataset:
        valset = args.Dataset.FSD(root=args.dataroot, phase='val', index=class_new, k=None, args=args)
    elif args.dataset in ['f2n', 'f2l', 'n2f', 'n2l', 'l2f', 'l2n']:
